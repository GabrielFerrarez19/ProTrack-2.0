...
export const getRelatorioLucroPeriodo = async (
  ...
  const [rows]: any = await db.query(sql, [dataInicio, dataFim]);
...
export const getRelatorioCompleto = async (
  ...
  const [
    ...
    getRelatorioLucroPeriodo(dataInicio, dataFim),
    getDistribuicaoMargemLucro(),
    getRelatorioEstoqueInvestimento(),
  ]);

  // Calcular resumo geral
  const receitaTotal = categoriasLucro.reduce(
    (total, cat) => total + cat.valor_total_venda,
    0
  );
  const custoTotal = categoriasLucro.reduce(
    (total, cat) => total + cat.valor_total_investido,
    0
  );
  const lucroTotal = receitaTotal - custoTotal;
  const margemLucroGeral =
    receitaTotal > 0 ? (lucroTotal / receitaTotal) * 100 : 0;
  const quantidadeVendas = evolucaoMensal.reduce(
    (total, periodo) => total + periodo.quantidade_vendas,
    0
  );
